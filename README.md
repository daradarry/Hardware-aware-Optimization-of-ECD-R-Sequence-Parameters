...
where $\ell$ is the number of blocks, $\ket{s_\ell}=U_\ell\cdots U_1\ket{0}$,  $\hat{A}$ is the arbitrary unitary, $||\hat{O}||_F^2= \text{Tr}(\hat{O}^\dagger\hat{O})/d$ is the weighted Frobenius norm and  $\hat{P}_d$  is the projection unto computational subspace of dimension $d$. For $b$ buffer levels, the total dimension $d_T=b+b$ and $\{\hat{Q}_k, k= 0,1,\ldots\}$ are projections onto the Fock state  

$\{\ket{k}\}$ (i.e. the Fock levels in the conditional displacement operators composed in $U_{ECD}$ are all truncated to $d_T$, hence, $U_{ECD} \in \mathbb{C}^{2d_T\times 2d_T}$).


$U_{ECD}$ is defined as follows
